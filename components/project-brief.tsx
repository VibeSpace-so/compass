...
const TYPE_CONFIG: Record<MemoryType, { label: string; icon: React.ReactNode; color: string }> = {
  ...
  context: { label: "Context", icon: <BookOpen className="w-3.5 h-3.5" />, color: "text-[var(--accent)]" },
  ...
  if (memories.length === 0 && !doc) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center px-6 py-12">
        <Brain className="w-12 h-12 text-[var(--text-faint)] mb-4" />
        <h3 className="text-[var(--text-secondary)] font-medium mb-2">No memories yet</h3>
    ...
    <div className="flex flex-col gap-4 p-3 sm:p-4 overflow-y-auto h-full mobile-scroll">
      <div className="flex items-center gap-2 mb-1">
        ...
        <div className="ml-auto flex gap-2">
          <button onClick={copyMarkdown} className="flex items-center gap-1 text-[10px] text-[var(--text-muted)] hover:text-[var(--accent)]" title="Copy">
            {copied ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />} Copy
          </button>
          <button onClick={exportMarkdown} className="flex items-center gap-1 text-[10px] text-[var(--text-muted)] hover:text-[var(--accent)]" title="Export">
              ...
              <div key={type} className="space-y-1.5">
                ...
                {typeMemories.map((memory) => (
                  <div key={memory.id} className="group flex items-start gap-2 pl-5 py-1">
                    ...
                    <span className="text-[10px] text-[var(--text-faint)] whitespace-nowrap">{new Date(memory.updatedAt ?? memory.createdAt).toLocaleDateString()}</span>
                    ...
                    {onUpdateMemoryTags && editingTags === memory.id ? (
                      ...
                      <button onClick={() => { setEditingTags(memory.id); setTagValue((memory.tags ?? []).join(", ")); }} className="flex flex-wrap gap-1 text-left" title="Edit tags">
                        {(memory.tags ?? []).length > 0
                          ...
                          : <span className="text-[10px] text-[var(--text-muted)]">Add tags</span>}
                      </button>
                    )}
                  </div>
                ))}
